...
`src/assistant/query_forecast.py` lets you ask about the forecast in plain English instead of calling `/predict` directly - it's grounded entirely in the real, live API above, never inventing numbers itself. Requires a free [Google AI Studio](https://aistudio.google.com/) API key in a local `.env` file (`GEMINI_API_KEY=...`, gitignored).
